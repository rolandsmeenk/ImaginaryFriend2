...
/// <summary>
/// Copied from the MRTK 
/// -Larger voice enum based on what is available on HoloLens, extra voices have to be installed if on Windows 10 if you want to run it on Windows but it will only work when built as UWP app
/// -Voice is pitched up
/// -Removed the RequiredComponent so the AudioClip can be at a different gameobject
/// </summary>
namespace Timmy
{
    // <summary>
    ...
    /// </remarks>
    public class TimmyTextToSpeech : MonoBehaviour
    {
        ...
        private void Awake()
        {
            try
            {
                ...
                // Pitch it up for Timmy
                synthesizer.Options.AudioPitch = 2;
                synthesizer.Options.SpeakingRate = .9;                 
#endif
            }
            catch (Exception ex)
            {
